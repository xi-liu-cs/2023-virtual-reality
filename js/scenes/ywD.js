...
export const init = async model =>
{
     ...
     */
    // let HUD = model.add();
    // HUD.add('cube').color(.3,.1,.05);//brown
    // HUD.add('cube').color(.05,.8,.1);//green
    // HUD.add('cube').color(.2,.2,.2);//gray
    // HUD.add('cube').color(1,1,1).opacity(.2);//white
    ...
    ladder1.translation = [0,0,0].map((x)=> x * v_near);
    ladder1.scale = [20,20,20].map((x)=> x * v_near);
    ...
    let gltfs4 = new Gltf2Node({url: './media/gltf/map_pin/scene.gltf'});
    ...
    gltfs4.translation = [0,0,0].map((x)=> x * v_far);
   ...
   `);
    ...
    model.move(0,1.5,0).scale(.6).animate(() => {
        ...
        if (leftTrigger && rightTrigger){
            ...
            gltfs4.translation = cg.add(gltfs3.translation,[0,18,0]);
        }
        else if (rightTrigger){
            gltf0.translation =cg.add(cg.scale(gltf1.translation, -.1),[4,-1,2]);
            gltfs4.translation = cg.add(gltfs1.translation,[0,18,0]);
        }
        else if (leftTrigger){
            gltf0.translation =cg.add(cg.scale(gltf2.translation, -.1),[-4,-.8,1.8]);
            gltfs4.translation = cg.add(gltfs2.translation,[0,18,0]);
        }
         ...
         * HUD configuration
         */
        // HUD.child(0).identity().move(gltfs1.translation);
        // HUD.child(2).identity().move(gltfs2.translation);
        // HUD.child(3).identity().move(gltfs3.translation);
        // HUD.hud().scale(.2,.2,.0001);
